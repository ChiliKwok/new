...
const getOfflineNarrative = (scenario: string, locationName: string, weather: string) => {
    return {
        title: "江湖传闻",
        description: `【${locationName}】\n时值${weather}。\n${scenario.split('(')[0]}。\n(API限流/离线模式：请DM根据题目自行演绎剧情与判定)`,
        dmNotes: "系统提示：API 配额已耗尽或未连接，已切换至离线题库模式。请DM直接根据标题进行主持。"
    };
};
  ...
  inputValue: number
    ...
    if (!process.env.API_KEY) {
        return { 
            ...
            eventText: `(离线模式) 第${day}日，${weather}。${SECTS[sectState.id].name}行至${location.name}。${location.desc}`, 
            effectSummary: "行军途中" 
        };
    }
    ...
    try {
        ...
        return {
            locationName: location.name,
            eventText: parts[1]?.trim() || `冒着${weather}继续赶路，抵达${location.name}。`,
        ...
        return { 
            locationName: location.name, 
            eventText: `(API限流/忙碌) 第${day}日，${weather}。众人行至${location.name}。${location.desc}`, 
            effectSummary: "继续前行" 
        };
    }
}
...
export const generateConflictNarrative = async (
    ...
    try {
        ...
        return response.text?.trim() || "两派对峙，气氛紧张。";
    } catch (e) {
        console.error("Gemini API Error:", e);
        return `在${location}，两派人马狭路相逢。${weather}之下，气氛肃杀，似有一触即发之势。`;
    }
}
...
export const generateOpportunityEvent = async (
    ...
    if (!process.env.API_KEY) {
        const offline = getOfflineNarrative(scenario, location.name, weather);
        return { title: offline.title, description: offline.description + "|||" + offline.dmNotes };
    }
    ...
    **注意：结局只能包含【属性变化】或【里程变化】，无道具。**
    ...
    try {
        ...
        const parts = (response.text || "").split('|||');
        return {
            title: parts[0]?.trim() || "江湖奇遇",
            description: (parts[1]?.trim() || `在${weather}中，偶遇奇事。`) + "|||" + (parts[2]?.trim() || "请DM自行裁决。")
        };
    } catch (e) {
        // Fallback for quota error: Use the raw scenario text
        console.error("Gemini API Error:", e);
        const offline = getOfflineNarrative(scenario, location.name, weather);
        return { title: offline.title, description: offline.description + "|||" + offline.dmNotes };
    }
