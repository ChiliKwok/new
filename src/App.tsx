...
const App: React.FC = () => {
    ...
    return (
       <div className="fixed inset-0 z-[100] bg-stone-950/95 backdrop-blur-sm flex items-center justify-center p-4">
           <div className="bg-stone-900 border border-gold/30 p-8 max-w-md text-center shadow-2xl space-y-6 rounded-lg">
               ...
               <p className="text-stone-500 text-sm">如无 Key，游戏将以离线随机事件模式运行。</p>
            ...
            >
              ...
              <div className="text-center w-full">
                  ...
                  <div className={`text-[10px] font-mono font-bold ${
                      state.lastMoveDesc.includes('+') ? 'text-green-400' : 
                      state.lastMoveDesc.includes('-') ? 'text-crimson' : 'text-stone-500'
                  }`}>
                      {state.lastMoveDesc}
                  </div>
              </div>
              ...
              <div className="absolute bottom-full mb-4 opacity-0 group-hover:opacity-100 transition-all duration-500 ease-out transform translate-y-4 group-hover:translate-y-0 pointer-events-none z-50">
                  <div className="w-64 bg-stone-900 border border-gold/40 rounded-lg shadow-[0_10px_30px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col">
                      <div className="w-full aspect-video bg-stone-950 relative">
                           {portraitImg ? (
                               <img src={portraitImg} className="w-full h-full object-cover opacity-90" />
                           ) : (
                               <div className="w-full h-full flex items-center justify-center text-stone-700 text-xs italic">暂无立绘</div>
                           )}
        ...
        >
            ...
            {gameState.customPath && gameState.customPath.length > 1 && (
                <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-50 translate-z-5">
                    <polyline 
                        points={gameState.customPath.map(p => `${p.x}%,${p.y}%`).join(' ')} 
                        fill="none" 
                        stroke="#c5a059" 
                        strokeWidth="1.5" 
                        strokeDasharray="4,4"
            ...
            {SECT_ORDER.map(sectId => {
                ...
                return (
                    <div 
                        key={sectId}
                        className={`
                            absolute flex flex-col items-center transition-all duration-1000 ease-in-out preserve-3d
                            ${isActive ? 'z-30' : 'z-10'}
                        `}
                        style={{ 
                            left, 
                            top, 
                            transform: `translate(-50%, -100%) translateZ(${isActive ? '50px' : '10px'})` // Elevate Z
                        }}
                    >
                         <div 
                            className={`
                                relative origin-bottom transition-transform duration-300
                                ${isActive ? 'scale-125' : 'scale-100 opacity-90'}
                            `}
                            style={{ transform: 'rotateX(-35deg)' }} 
                         >
                             <div className={`
                                w-10 h-10 rounded border-2 shadow-xl overflow-hidden bg-stone-800
                                ${isActive ? 'border-gold ring-2 ring-gold/40' : `border-stone-500`}
                             `}>
                                {customImg ? (
                                    <img src={customImg} className="w-full h-full object-cover" alt={sect.name} />
                                ) : (
                                    <div className={`w-full h-full flex items-center justify-center text-xs font-bold ${sect.color} bg-stone-900`}>{sect.name[0]}</div>
                                )}
                             </div>
                             
                             <div className={`
                                absolute -top-5 left-1/2 -translate-x-1/2 
                                bg-black/80 text-[8px] text-gold px-1 rounded whitespace-nowrap border border-gold/20
                         ...
                         <div 
                            className="absolute bottom-0 w-8 h-3 bg-black/60 blur-sm rounded-full pointer-events-none"
                            style={{ 
                                transform: `translateY(50%) rotateX(0deg) scale(${isActive ? 1.2 : 0.8})`,
                 ...
                 <div className="mb-6">
                    <div className="flex justify-between items-start mb-2">
                        <div>
                            ...
                            <div className="text-xs text-stone-500 font-serif flex items-center gap-1">
                                <span>📍</span> {activeState.currentLocationName} ({activeState.locationProgress}里)
                            </div>
                        </div>
                    ...
                    <div className="w-full aspect-video bg-stone-950 rounded border border-stone-800 overflow-hidden relative shadow-lg mb-2 group">
                        {activePortrait ? (
                            <img src={activePortrait} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                        ) : (
                            <div className="w-full h-full flex flex-col items-center justify-center text-stone-700 gap-2">
                                <span className="text-4xl opacity-20">❖</span>
                                <span className="text-xs italic">暂无立绘</span>
                            </div>
                        )}
                        <div className="absolute inset-0 bg-gradient-to-t from-stone-900/80 via-transparent to-transparent pointer-events-none"></div>
                        <div className="absolute bottom-2 right-2 flex items-center gap-1 bg-black/40 px-2 py-0.5 rounded backdrop-blur-sm border border-white/10">
                             <span className="text-lg">🌤️</span>
                             <span className="text-xs text-stone-300 font-serif">{gameState.weather.split(' - ')[0]}</span>
                        </div>
                    </div>
                 </div>
                    ...
                    <div>
                         ...
                         <div className="flex gap-2">
                             <input 
                                ...
                                className="flex-1 bg-stone-950 border border-stone-600 text-gold text-xl font-serif p-2 rounded focus:border-gold focus:outline-none text-center"
                             />
                             <div className="flex flex-col justify-center text-xs text-stone-500">里</div>
                         </div>
                    </div>
                    <Button onClick={handleTurnStart} disabled={loading} className="w-full py-3 text-lg tracking-[0.2em]">
                        {loading ? '演化中...' : '行动'}
                    </Button>
                 </div>
                    ...
                    {Object.keys(logsByDay).sort((a,b) => Number(b) - Number(a)).map((dayStr) => {
                        ...
                        return (
                            <div key={day} className="relative pl-4 border-l border-stone-800">
                                <div className="absolute -left-[11px] top-0 w-5 h-5 bg-stone-900 border border-stone-600 rounded-full flex items-center justify-center text-[10px] text-gold font-bold">
                                    {day}
                                </div>
                                <div className="space-y-3 pt-1">
                                    {entries.map((entry, idx) => (
                                        <div key={idx} className={`p-3 rounded-r border-l-2 text-sm leading-relaxed ${
                                            entry.type === 'move' ? 'border-stone-600 bg-stone-900/40 text-stone-300' :
                                            entry.type === 'conflict' ? 'border-crimson bg-crimson/5 text-stone-200' :
                                            entry.type === 'event' ? 'border-gold bg-gold/5 text-stone-200' :
                                            'border-stone-500 text-stone-500 italic'
                                        }`}>
                                            {entry.content}
                                        </div>
                                    ))}
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="w-full max-w-2xl bg-stone-900 border border-gold shadow-2xl rounded-lg overflow-hidden animate-fade-in-up flex flex-col max-h-[90vh]">
                  <div className="bg-stone-950 p-4 border-b border-stone-800 flex justify-between items-center shrink-0">
                      <h2 className="text-xl text-gold font-serif font-bold tracking-widest">
                          {interaction.type === 'PVP' ? '⚔️ 狭路相逢' : '✨ 江湖奇遇'}
                      </h2>
                      <div className="text-stone-500 text-xs font-serif">{interaction.locationName}</div>
                  </div>
                  
                  <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                      <div className="text-stone-200 text-base leading-relaxed py-6 px-8 border-y-2 border-stone-800 bg-stone-950/50 mb-6 font-serif tracking-wide whitespace-pre-wrap">
                          {narrative}
                      </div>
                      ...
                      {dmNotes && interaction.type === 'OPPORTUNITY' && (
                          <div className="mb-6 bg-stone-950 p-4 rounded border border-stone-800">
                              ...
                              <div className="text-xs text-stone-400 font-mono whitespace-pre-wrap leading-relaxed pl-2 border-l-2 border-gold/30">
                                  {dmNotes}
                              </div>
                          </div>
                      )}
                      ...
                      <div className="bg-stone-800/30 p-4 rounded border border-gold/30">
                          ...
                          <div className="flex items-center justify-center gap-4 mb-6">
                              <div className="flex flex-col items-center">
                                ...
                                <div className="flex items-center">
                                    <input 
                                        ...
                                        className="w-20 bg-stone-950 border border-gold text-gold text-xl p-2 rounded text-center focus:outline-none font-bold"
                                    />
                              ...
                              {interaction.type === 'OPPORTUNITY' && (
                                <div className="flex gap-2">
                                    <button 
                                        onClick={() => setApplySkipTurn(!applySkipTurn)}
                                        className={`px-2 py-2 text-xs rounded border transition-colors ${applySkipTurn ? 'bg-crimson text-white border-crimson' : 'border-stone-600 text-stone-400'}`}
                                    >
                                        🛑 下回停滞
                                    </button>
                                    <button 
                                        onClick={() => setApplyActionAgain(!applyActionAgain)}
                                        className={`px-2 py-2 text-xs rounded border transition-colors ${applyActionAgain ? 'bg-jade text-white border-jade' : 'border-stone-600 text-stone-400'}`}
                                    >
                                        ⏩ 再行一程
                                    </button>
                                </div>
                          ...
                          {interaction.type === 'OPPORTUNITY' && (
                            <div className="grid grid-cols-4 gap-4 text-xs text-stone-400">
                                <div className="space-y-1">
                                    <div className="text-center font-bold">武力变化</div>
                                    <input type="number" className="w-full bg-stone-950 border border-stone-600 rounded p-2 text-center text-stone-200" 
                                        value={interactionStats.martial} onChange={(e) => setInteractionStats({...interactionStats, martial: parseInt(e.target.value) || 0})} />
                                </div>
                                <div className="space-y-1">
                                    <div className="text-center font-bold">智谋变化</div>
                                    <input type="number" className="w-full bg-stone-950 border border-stone-600 rounded p-2 text-center text-stone-200" 
                                        value={interactionStats.strategy} onChange={(e) => setInteractionStats({...interactionStats, strategy: parseInt(e.target.value) || 0})} />
                                </div>
                                <div className="space-y-1">
                                    <div className="text-center font-bold">财力变化</div>
                                    <input type="number" className="w-full bg-stone-950 border border-stone-600 rounded p-2 text-center text-stone-200" 
                                        value={interactionStats.wealth} onChange={(e) => setInteractionStats({...interactionStats, wealth: parseInt(e.target.value) || 0})} />
                                </div>
                                <div className="space-y-1">
                                    <div className="text-center font-bold">威望变化</div>
                                    <input type="number" className="w-full bg-stone-950 border border-stone-600 rounded p-2 text-center text-stone-200" 
                                        value={interactionStats.prestige} onChange={(e) => setInteractionStats({...interactionStats, prestige: parseInt(e.target.value) || 0})} />
                                </div>
                            </div>
                          )}
                      </div>
                      ...
                      <div className="mt-6 flex gap-4">
                        {interaction.type === 'PVP' && interaction.targetSectId ? (
                            <>
                                <Button onClick={() => resolvePvP(interaction.activeSectId, 'BATTLE')} variant="danger" className="flex-1 text-xs py-3">
                                    {activeSect.name} 胜 (对方退)
                                </Button>
                                <Button onClick={() => resolvePvP(interaction.activeSectId, 'NEGOTIATE')} variant="secondary" className="flex-1 text-xs py-3">
                                    {activeSect.name} 避 (自愿退)
                                </Button>
                                <div className="w-px bg-stone-700 mx-2"></div>
                                <Button onClick={() => resolvePvP(interaction.targetSectId!, 'BATTLE')} variant="danger" className="flex-1 text-xs py-3">
                                    {SECTS[interaction.targetSectId!].name} 胜
                                </Button>
                                <Button onClick={() => resolvePvP(interaction.targetSectId!, 'NEGOTIATE')} variant="secondary" className="flex-1 text-xs py-3">
                                    {SECTS[interaction.targetSectId!].name} 避
                                </Button>
                                <Button onClick={() => resolvePvP(interaction.activeSectId, 'COOP')} className="flex-1 text-xs py-3 bg-jade border-jade text-white hover:bg-green-700">
                                    🤝 联手共进
                                </Button>
                            </>
                        ) : (
                            <>
                                <Button onClick={() => resolveOpportunity(true, 'FORWARD')} className="flex-1 text-stone-900 bg-gold hover:bg-yellow-400 font-bold py-3 text-lg">
                                    大吉 (结算)
                                </Button>
                                <Button onClick={() => resolveOpportunity(false, 'BACKWARD')} variant="ghost" className="flex-1 border border-stone-600 hover:bg-stone-800 text-stone-400 py-3">
                                    大凶 (结算)
                                </Button>
                            </>
                        )}
  ...
  // Api Key Modal (omitted for brevity, same as before)
  const renderApiKeyModal = () => {
    if (!showApiKeyModal) return null;
    return (
       <div className="fixed inset-0 z-[100] bg-stone-950/95 backdrop-blur-sm flex items-center justify-center p-4">
      ...
      return (
          <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex justify-end">
               <div className="w-full max-w-md h-full bg-stone-900 border-l border-gold shadow-2xl flex flex-col animate-slide-in-right relative">
                   ...
                   {/* Portrait Header 16:9 */}
                   <div className="w-full aspect-video relative bg-stone-950 shrink-0">
                       {portraitImg ? (
                           <img src={portraitImg} className="w-full h-full object-cover mask-gradient-b" />
                       ) : (
                           <div className="w-full h-full flex items-center justify-center bg-stone-800 text-stone-600 italic">暂无立绘</div>
                       )}
                       <div className="absolute inset-0 bg-gradient-to-t from-stone-900 via-transparent to-transparent"></div>
                   ...
                   <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-stone-900 custom-scrollbar">
                       <div className="grid grid-cols-4 gap-2 text-center p-4 bg-stone-950 rounded border border-stone-800">
                           <div>
                               <div className="text-stone-500 text-xs mb-1">武力</div>
                               <div className="text-xl text-gold font-serif">{state.stats.martial}</div>
                           </div>
                           <div>
                               <div className="text-stone-500 text-xs mb-1">智谋</div>
                               <div className="text-xl text-gold font-serif">{state.stats.strategy}</div>
                           </div>
                           <div>
                               <div className="text-stone-500 text-xs mb-1">财富</div>
                               <div className="text-xl text-gold font-serif">{state.stats.wealth}</div>
                           </div>
                           <div>
                               <div className="text-stone-500 text-xs mb-1">威望</div>
                               <div className="text-xl text-gold font-serif">{state.stats.prestige}</div>
                           </div>
                       </div>
                       ...
                       <div className="space-y-2">
                           ...
                           <div className="flex flex-wrap gap-2">
                               {state.visitedLocations.map((loc, i) => (
                                   <div key={i} className="flex items-center text-xs text-stone-400 bg-stone-950 px-2 py-1 rounded border border-stone-800">
                                       {i > 0 && <span className="mr-1 text-stone-600">→</span>}
                                       {loc}
                                   </div>
                               ))}
                           </div>
                       ...
                       <div className="space-y-4">
                            ...
                            {state.history.length === 0 ? <div className="text-stone-600 text-sm">暂无记录</div> : state.history.map((entry, i) => (
                                <div key={i} className="text-stone-300 text-sm border-l border-stone-700 pl-4 py-1 relative">
                                    <div className="absolute -left-[5px] top-2 w-2 h-2 rounded-full bg-stone-500"></div>{entry}
                                </div>
                            ))}
                       </div>
                   </div>
        ...
        >
            ...
            {SECT_ORDER.map(sectId => {
                ...
                return (
                    ...
                    >
                         {/* Billboard: Counter-rotate the marker so it stands up */}
                         <div 
                            className={`
                                relative origin-bottom transition-transform duration-300
                                ${isActive ? 'scale-125' : 'scale-100 opacity-90'}
                         ...
                         {/* Shadow on the ground (Projected down) */}
                         <div 
                            className="absolute bottom-0 w-8 h-3 bg-black/60 blur-sm rounded-full pointer-events-none"
                            style={{ 
                                transform: `translateY(50%) rotateX(0deg) scale(${isActive ? 1.2 : 0.8})`,
      ...
      return (
          <div className="flex-1 flex flex-col bg-stone-950 p-8 overflow-hidden">
              ...
              <div className="flex-1 flex gap-8 min-h-0">
                  {/* Left Column: Map Editor (21:9 Forced) */}
                  <div className="flex-1 flex flex-col gap-4">
                      <div className="flex justify-between items-center">
                          ...
                          <div className="flex gap-2">
                              ...
                              <Button 
                                onClick={() => { setIsDrawingPath(!isDrawingPath); if(!isDrawingPath) setTempPath([]); }} 
                                variant={isDrawingPath ? 'danger' : 'secondary'} 
                                className="text-xs py-1"
                              >
                                {isDrawingPath ? '结束绘制' : '重绘路径'}
                              </Button>
                          </div>
                      </div>
                          ...
                          >
                               ...
                               {tempPath.length > 0 && (
                                   <svg className="absolute inset-0 w-full h-full pointer-events-none">
                                       ...
                                       />
                                       {tempPath.map((p, i) => (
                                           <circle key={i} cx={`${p.x}%`} cy={`${p.y}%`} r="3" fill="#c5a059" />
                                       ))}
                                   </svg>
                               )}
                               
                               {isDrawingPath && (
                                   <div className="absolute top-4 left-4 bg-black/70 text-white px-3 py-1 rounded text-xs pointer-events-none">
                                       点击地图添加路径点 ({tempPath.length})
                                   </div>
                               )}
                          </div>
                      </div>
                  ...
                  {/* Right Column: Sect Customization */}
                          ...
                          {SECT_ORDER.map(sectId => {
                              ...
                              return (
                                  <div key={sectId} className="bg-stone-900 p-3 rounded border border-stone-800 flex gap-3 items-center">
                                      <div 
                                        onClick={() => handleSectImageClick(sectId, 'TOKEN')}
                                        className="w-12 h-12 rounded border border-stone-700 bg-stone-800 flex items-center justify-center overflow-hidden cursor-pointer hover:border-gold relative group"
                                      >
                                      ...
                                      <div 
                                        onClick={() => handleSectImageClick(sectId, 'PORTRAIT')}
                                        className="w-20 aspect-video rounded border border-stone-700 bg-stone-800 flex items-center justify-center overflow-hidden cursor-pointer hover:border-gold relative group"
                                      >
